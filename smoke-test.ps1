...
$output = $request | node .\node_modules\@earendil-works\pi-coding-agent\dist\cli.js --mode rpc --provider openai-codex --model gpt-5.5 --thinking medium --approve
...
$copilotOutput = $request | node .\node_modules\@earendil-works\pi-coding-agent\dist\cli.js --mode rpc --provider github-copilot --model gpt-5.3-codex --thinking medium --approve
...
node .\permission-smoke.mjs
if ($LASTEXITCODE -ne 0) { throw 'Permission policy smoke test failed.' }

node .\tool-smoke.mjs
if ($LASTEXITCODE -ne 0) { throw 'Built-in tool execution smoke test failed.' }
